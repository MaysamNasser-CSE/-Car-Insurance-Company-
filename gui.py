...
def setup_delete_insurancepolicy_gui(root, parent_root, db_connection):
    ...
    try:
        ...
        print(f"Error opening the image: {e}")
def delete_coveredperson_action(coveredperson_id, db_connection, child_root, parent_root):
    success, message = delete_coveredperson_id(db_connection, coveredperson_id)
    if success:
        messagebox.showinfo("Success", message)
        child_root.destroy()
        parent_root.deiconify()
